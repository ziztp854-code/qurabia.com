...
## Codex/Sol project integration

- Resolve `<skill-root>` to this skill's directory. Every legacy `~/.claude/skills/design` example below maps to `<skill-root>` in Codex/Sol.
- `brand` and `design-system` decisions have higher priority than this skill's generic visual suggestions.
- External AI generation, packages, credentials, and related skills are optional dependencies. Never install, call, or configure them unless the current user task explicitly requires it.
- Graphic composition must not alter application architecture or game logic.

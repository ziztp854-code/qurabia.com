...
- Resolve `<skill-root>` to this skill's directory. Every legacy `${CLAUDE_PLUGIN_ROOT}/.claude/skills/ui-ux-pro-max` example below maps to `<skill-root>` in Codex/Sol.
- An existing project design system overrides generated recommendations. Do not persist or force a replacement design system unless the current user task explicitly requests it.
- Repository instructions, brand rules, and game-logic boundaries always take precedence over search results.

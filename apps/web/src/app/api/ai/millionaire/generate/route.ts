...
import {
  ...
  millionaireGenerationInputSchema,
} from '@/lib/ai/millionaire-generation';
...
export async function POST(request: Request) {
  ...
  const parsed = millionaireGenerationInputSchema.safeParse(body.value);
  if (!parsed.success) {
    return NextResponse.json(
  ...
  const allowed =
    (await checkRateLimit(`ai-millionaire-user:${sessionUser.id}`, 2, MINUTE)) &&
    (await checkRateLimit(`ai-millionaire-ip:${requestIp}`, 8, HOUR)) &&
    (await checkRateLimit('ai-millionaire-global:daily', 200, DAY));
  if (!allowed) {
    return NextResponse.json(
  ...
  try {
    const ladder = await generateMillionaireLadder(parsed.data);
    return NextResponse.json(
      { ok: true, ladder },
      { headers: { 'cache-control': 'no-store, max-age=0' } },
    );
  } catch (error) {
    const known = error instanceof MillionaireGenerationError ? error : null;
    return NextResponse.json(
      { ok: false, message: known?.message || 'تعذّر توليد السلم الآن.' },

...
export type LadderHostAccessPayload = {
  hostId: string;
  expiresAt: number;
};
...
export function createLadderHostAccessToken(secret: string, payload: LadderHostAccessPayload) {
  if (secret.length < 16) {
    throw new Error('Ladder host token secret must contain at least 16 characters.');
  }
...
export function verifyLadderHostAccessToken(
  secret: string,
  token: string,
  now = Date.now(),
): LadderHostAccessPayload | null {
  ...
  try {
    ...
    ) as Partial<LadderHostAccessPayload>;
    if (
    ...
    return payload as LadderHostAccessPayload;
  } catch {
    return null;
  }
}
